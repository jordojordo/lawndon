...
void fresh_link(struct MyLink *p, uint16_t addr, float range, float dbm)
{
    ...
    if (temp != NULL) {
        ...
        if (temp->range[0] > 0) { sumRanges += temp->range[0]; validRanges++; }
        if (temp->range[1] > 0) { sumRanges += temp->range[1]; validRanges++; }
        if (temp->range[2] > 0) { sumRanges += temp->range[2]; validRanges++; }
...
void make_link_json(struct MyLink *p, String *s)
{
    ...
    *s = "{\"links\":[";
    ...
    while (temp->next != NULL)
    {
        temp = temp->next;
        char link_json[50];
        sprintf(link_json, "{\"A\":\"%X\",\"R\":\"%.1f\"}", temp->anchor_addr, temp->averageRange);
        *s += link_json;
        if (temp->next != NULL)
        {
            *s += ",";
